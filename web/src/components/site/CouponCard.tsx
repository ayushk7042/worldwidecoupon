  ...
  return (
    <article
      className={classNames(
        "@container surface group relative overflow-hidden rounded-2xl border shadow-[var(--shadow-card)] transition-all duration-200 hover:-translate-y-1 hover:border-brand-300 hover:shadow-[var(--shadow-lift)]",
        ribbon ? "border-[var(--border-subtle)]" : "border-[var(--border-subtle)]",
    ...
    >
      {ribbon ? (
        <p
          className={classNames(
            "px-4 py-1.5 text-[10px] font-extrabold uppercase tracking-[0.16em]",
            ribbon.className
          )}
        >
          {ribbon.label}
      ...
      <div className="flex flex-col gap-3 p-3.5 @2xl:flex-row @2xl:items-center @2xl:gap-4">
        {/* ---- what it saves ---- */}
        <div className="flex shrink-0 items-center gap-2 @2xl:w-[7.5rem] @2xl:flex-col @2xl:items-center @2xl:gap-0.5 @2xl:self-stretch @2xl:justify-center @2xl:border-r @2xl:border-dashed @2xl:border-[var(--border-strong)] @2xl:pr-4 @2xl:text-center">
          ...
          <span className="font-display text-xl font-extrabold leading-tight text-brand-600 @2xl:text-[1.7rem]">
            {coupon.badge}
          </span>
        </div>
        ...
        <div className="min-w-0 flex-1">
          {showStore && store ? (
            <Link
              ...
              className="mb-1 flex items-center gap-2 text-[11px] font-bold text-faint transition hover:text-brand-600"
            >
              <StoreLogo name={store.name} logo={store.logo} size={22} rounded="rounded-md" />
              {store.name}
            </Link>
          ) : null}
          ...
          {lines.length || coupon.terms ? (
            <details className="group/details mt-1">
              ...
              <p className="mt-2 text-[11px] font-semibold text-faint">
                ...
                {coupon.verified ? " · checked by an editor" : ""}
              </p>
            </details>
          ) : (
            <p className="mt-1 text-[11px] font-semibold text-faint">{expiry ?? "No expiry date"}</p>
          )}
        </div>
